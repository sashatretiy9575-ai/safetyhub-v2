...
import { PHONE_COUNTRY_CODES, isPhoneCountryCode } from '../../lib/phone/country-codes.ts';

test('lightweight browser country validation matches installed phone metadata', () => {
  assert.deepEqual([...PHONE_COUNTRY_CODES], getCountries());
  for (const code of getCountries()) assert.equal(isPhoneCountryCode(code), true);
  for (const invalid of ['', 'ZZ', 'kz', '__proto__'])
    assert.equal(isPhoneCountryCode(invalid), false);
});
...
test('international phone picker keeps Kazakhstan, Russia, and China first', () => {
  ...
  assert.deepEqual(
    phoneCountryOptions()
      .slice(0, 3)
      .map((option) => option.countryIso2),
    ['KZ', 'RU', 'CN'],
  );
});

test('country labels are serialized by server pages instead of recomputed during hydration', async () => {
  const [phoneInput, onboardingPage, onboardingForm, profilePage, adminAccountPage, profileForm] =
    await Promise.all([
...
test('server phone normalization stores selected country and E.164 only', () => {
  assert.deepEqual(normalizeUserPhone({ countryIso2: 'KZ', nationalNumber: '701 729 0349' }), {
    countryIso2: 'KZ',
    phoneE164: '+77017290349',
  });
  assert.deepEqual(normalizeUserPhone({ countryIso2: 'CN', nationalNumber: '138 0013 8000' }), {
    countryIso2: 'CN',

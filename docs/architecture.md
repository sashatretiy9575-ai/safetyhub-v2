...
пишется потоком в выбранный файл (только когда выбрана одна компания); иначе
браузер группирует метаданные по компании и получает отдельный архив на каждую
компанию (`<компания>.zip`, без названия — `safetyhub-certificates-<дата>.zip`),
не более 100 сертификатов в каждом. Прогресс и отмена остаются клиентскими, PDF/ZIP

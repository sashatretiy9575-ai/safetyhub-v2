...
export function BottomTabBar({ accountMode }: { accountMode: AccountMode }) {
  ...
  const tabs = [
    { href: ROUTES.home, icon: <House size={21} weight="regular" />, label: 'Главная' },
    { href: ROUTES.topics, icon: <BookOpen size={21} weight="regular" />, label: 'Курсы' },
    { href: ROUTES.blog, icon: <Article size={21} weight="regular" />, label: 'Блог' },
    { href: ROUTES.contacts, icon: <Phone size={21} weight="regular" />, label: 'Контакты' },
    accountMode === 'guest'
      ? { ...accountItem, icon: <SignIn size={21} weight="regular" /> }
      : {
          ...accountItem,
    ...
    >
      <div className="flex h-full items-stretch">
        {tabs.map(({ href, icon, label }) => {
          const isActive = pathname !== null && isRouteActive(pathname, href);
          return (
            <Link
              key={href}
              href={href}

...
begin
  ...
  -- The access token issued before the reset is also denied immediately, not
  -- merely when GoTrue attempts the next refresh.
  perform set_config('request.jwt.claim.sub', v_user_id::text, true);
  perform set_config('request.jwt.claims', jsonb_build_object(
    'role', 'authenticated',
    'sub', v_user_id,
  ...
  if v_result is not null then
    raise exception 'pre-reset access token remained usable: %', v_result;
  end if;
  v_failed := false;
  begin
    perform private.require_active_user();
  exception when insufficient_privilege then
    if sqlerrm = 'SESSION_REAUTHENTICATION_REQUIRED' then
      v_failed := true;
    else
      raise;
    end if;
  ...
  if not v_failed then
    raise exception 'stale zh access token passed the active-user gate';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'request-time zh epoch helper grants are unsafe';
  end if;
end;
$test$;

...
begin
 ...
 update public.profiles set avatar_updated_at=now() where id=learner;
 changed:=public.verify_user_identity_with_education(learner,'Участник','Проверенный','Инженер','Document editor regression','Высшее');
 if changed ? '__safetyhubRpcError' then raise exception 'Education verification failed: %',changed; end if;
 if (select education from public.profiles where id=learner) <> 'Высшее' then raise exception 'Education not saved'; end if;
 select consumed into count_before from private.business_rate_limits where actor_id=actor and action='admin.identity.mutate';
 changed:=public.verify_user_identity_with_education(learner,'Другое','Проверенный','Инженер','Document editor regression',repeat('x',201));
 if not (changed ? '__safetyhubRpcError') then raise exception 'Invalid education accepted'; end if;
 if (select name from public.profiles where id=learner)<>'Участник' then raise exception 'Partial identity update survived education failure'; end if;
 if (select consumed from private.business_rate_limits where actor_id=actor and action='admin.identity.mutate') <> count_before+1 then raise exception 'Failed education change refunded quota'; end if;
 data:=public.get_document_editor_data('Document editor regression',test_slug);
 if not exists(select 1 from jsonb_array_elements(data->'participants') p where p->>'userId'=learner::text and p->>'education'='Высшее') then raise exception 'Education missing in protocol'; end if;
 if private.valid_document_defaults((settings->'documentDefaults')||'{"insertWidthCm":7,"insertHeightCm":10}'::jsonb) then raise exception 'Invalid size accepted'; end if;
 if not private.valid_document_defaults((settings->'documentDefaults')||'{"insertWidthCm":32,"insertHeightCm":10}'::jsonb) then raise exception 'Valid size rejected'; end if;

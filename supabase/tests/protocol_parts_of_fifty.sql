...
begin
 ...
 values('SH-PARTS-'||replace(gen_random_uuid()::text,'-',''),learner,r.id,att,a.id,1,'Тестовый Слушатель','Плотник',p_organization,r.slug,title,title,'ru',a.score,r.question_count,r.pass_score,a.completed_at,'manual') returning * into c;
 return c;
end; $$;

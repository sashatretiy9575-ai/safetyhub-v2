-- A deliberately narrow, capability-gated admin queue for the manual learner
-- approval lifecycle. It is separate from the historical certificate identity
-- workflow: approving this request authorizes course access; certificate
-- issuance still uses its own audited identity controls.

create table private.account_approval_decision_receipts (
  ...
  constraint account_approval_decision_receipts_request_hash
    check (request_hash ~ '^[0-9a-f]{64}$')
);

create index account_approval_decision_receipts_expiry_idx
  on private.account_approval_decision_receipts (expires_at);

-- Only an administrator who has the identity-management capability can read
-- phone and profile details in this queue. It intentionally excludes all
-- assessment content, answer keys, legal history, and broad audit payloads.
...
begin
  ...
  next_item as materialized (
    select approval_due_at, user_id
    from candidates
    order by approval_due_at asc, user_id asc
    offset v_limit
    limit 1
  ...
  select
    ...
    (select approval_due_at from next_item),
    (select user_id from next_item)
  into v_items, v_next_due_at, v_next_user_id
  from visible;

  return jsonb_build_object(
    'items', v_items,
    'total', v_total,
    'hasMore', v_next_user_id is not null,
    'nextCursor', case
      when v_next_user_id is null then null
      else jsonb_build_object('at', v_next_due_at, 'id', v_next_user_id)
    end
  );
end;
$$;
...
create function public.prune_account_approval_decision_receipts(p_limit integer default 500)
returns integer
language plpgsql
security definer
set search_path = ''
...
begin
  with expired as (
    select receipt.actor_user_id, receipt.idempotency_key
    from private.account_approval_decision_receipts receipt
    where receipt.expires_at < statement_timestamp()
    order by receipt.expires_at asc
...
revoke execute on function public.prune_account_approval_decision_receipts(integer)
  from public, anon, authenticated;
grant execute on function public.prune_account_approval_decision_receipts(integer)
  to service_role;

comment on function public.list_pending_account_approval_page(integer,timestamptz,uuid) is
  'Capability-gated, minimal-PII manual learner-approval queue. It exposes no course or answer-key data.';
comment on function public.decide_account_approval(uuid,uuid,text,text) is
  'Idempotent admin approval/rejection of a pending learner request. The 24-hour deadline never auto-approves.';

...
import pg from 'pg';
import { validateAndRenderPresentation } from './snapshot/presentation-pdf-qa.mjs';
import {
  buildLocalizedPublishedSnapshot,
  validateLocalizedPublishedSnapshot,
} from './localization/localized-published-snapshot.mjs';
import {
  assertLegacyRuContentContract,
  classifyLocalizedSchema,
} from './localization/linked-preflight-contract.mjs';
import { loadStage6PublicationBatch } from './localization/stage6-publication-contract.mjs';
import {
...
} from '../db/database-backup-security.mjs';
import {
...
function generateSeed({ articlesRoot, coursesRoot, mediaRoot, localizationsRoot, outputPath }) {
  const script = path.join(ROOT, 'scripts', 'content', 'generate-content-seed.mjs');
...
async function validateStagedSnapshot({ articlesRoot, coursesRoot, mediaRoot, localizationsRoot }) {
  const script = path.join(ROOT, 'scripts', 'content', 'snapshot', 'validate-snapshot.mjs');
  const result = spawnSync(
    process.execPath,
    [
      script,

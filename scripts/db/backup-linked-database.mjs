...
if (
  ...
  console.error(
    'Usage: node scripts/db/backup-linked-database.mjs --expected-project-ref <current-production-ref> --ssl-root-cert <absolute-Supabase-CA.pem> [--ssl-root-cert-sha256 <lowercase-sha256>] --output <new-directory> --pg-bin <PostgreSQL-17-bin> --recovery-key-output <new-file>',
  );
  process.exit(1);
}
...
try {
  ...
  runNodeScript('scripts/db/create-database-backup.mjs', [
    '--schema',
    schemaPath,
    '--data',
    dataPath,
  ...
  runNodeScript('scripts/db/restore-database-backup.mjs', [
    '--backup',
    outputDirectory,
    '--output',
